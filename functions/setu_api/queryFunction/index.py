...
def generate_answer(query_text: str, retrieved_records: list[dict], language: str) -> dict:
    ...
    try:
        ...
        # Build prompt context (limit to top 3 to prevent massive token payloads and timeouts)
        top_records = retrieved_records[:3]
        context = "\n".join([f"[{r.get('case_id', 'id')}] {r.get('narrative_en', '')}" for r in top_records])
        ...
        source_ids = list(set([r.get("case_id") for r in top_records if r.get("case_id")]))
